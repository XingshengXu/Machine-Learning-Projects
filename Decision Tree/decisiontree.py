...
def plot_node(node, px, py, dx, parent_x=None, parent_y=None):
    if node is None:
        return

    x = px + dx
    y = py - 0.3  # Increase y to provide more vertical space

    if parent_x is not None and parent_y is not None:
        # Connect parent and child with a line
        plt.plot([parent_x, x], [parent_y, y], 'k-')

    if node.feature_threshold is not None:
        feature = node.feature_threshold[0]
        threshold = node.feature_threshold[1]
        class_freq = ", ".join(
            [f"{i}:{freq}" for i, freq in enumerate(node.class_frequency)])
        text = (
        ...
        class_freq = ", ".join(
            [f"{i}:{freq}" for i, freq in enumerate(node.class_frequency)])
        plt.text(x, y, f"prediction: {node.prediction}\n{class_freq}",
                 verticalalignment='center',
                 bbox=dict(facecolor='red', alpha=0.5),
                 horizontalalignment='center',
                 fontsize=9)

    # Decrease dx to adjust horizontal distance
    # Adjusts the vertical distance for the left child node
    plot_node(node.left, px - dx * 1, y, dx * 0.5, x, y)
    # Adjusts the vertical distance for the right child node
    plot_node(node.right, px + dx * 1, y, dx * 0.5, x, y)


def plot_tree(tree):
    plt.figure(figsize=(10, 10))  # You can adjust the figure size as well
    plot_node(tree.root, 0.2, 0.5, 1.1)
